...
async function saveLocal(filename, buffer) {
  ...
  // Prevent path traversal: the resolved path must stay inside the uploads dir
...
// Cache the S3 client across calls (and lambda invocations via global) so we
// reuse TCP/TLS connections instead of paying a new handshake per operation.
function getR2Client() {
  if (!S3Client) return null;
  if (global.__r2Client !== undefined) return global.__r2Client;
  const accessKeyId = process.env.R2_ACCESS_KEY_ID;
  const secretAccessKey = process.env.R2_SECRET_ACCESS_KEY;
  const bucket = process.env.R2_BUCKET;
  if (!accessKeyId || !secretAccessKey || !bucket) {
    global.__r2Client = null;
    return null;
  }

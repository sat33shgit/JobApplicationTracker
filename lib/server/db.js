...
if (connectionString && process.env.FORCE_LOCAL_DB !== 'true') {
  ...
  if (!globalAny.__pgPool) {
    globalAny.__pgPool = new Pool({
      connectionString,
      // Serverless-friendly settings: keep few connections per instance and
      // release idle ones quickly so many warm lambdas don't exhaust Postgres.
  ...
  async function query(text, params) {
    try {
      ...
      // In production, surface the error — silently falling back to the in-memory
      // mock would make writes appear to succeed while data is lost.
      if (process.env.NODE_ENV === 'production') throw e;
      // In local dev, fallback to mock DB to keep dev usable
      console.warn('Postgres query failed, falling back to in-memory mock DB:', e?.message);
